...
const DoneCampaignScreen = () => {
    ...
    useEffect(() => {
        const getdata = () => {
          axios.get(` http://92.113.26.138:8081/api/user/done-campaign?userId=${items.authorization.userInfo.userOrBloger.id}`,
              {
                  headers: {
      ...
      return (
        ...
        <Header 
                    title={items.lang.currentLocal.settings && items.lang.currentLocal.settings.doneCampaigns}
    />
    ...
    <Text style={styles.btnText}>{items.lang.currentLocal.hom.seeMore}</Text>
...
export default DoneCampaignScreen
