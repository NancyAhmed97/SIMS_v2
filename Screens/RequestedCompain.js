...
const RequestedCompain = () => {
    ...
    useEffect(() => {
        ...
        const getdata = () => {
            axios.get(`http://92.113.26.138:8081/api/user/requested-campaign?userId=${items.authorization.userInfo.userOrBloger.id}`,
                {
                    headers: {
    ...
    return (
        <View style={[items.lang.currentLocal.language == 'ar' && { direction: 'rtl' }, styles.container]}>
            <Header
                title={items.lang.currentLocal.settings && items.lang.currentLocal.settings.requestedCampaigns}
...
export default RequestedCompain
